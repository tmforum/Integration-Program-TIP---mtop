...
public class TopologicalLinkT  implements Serializable {
    ...
    @JoinColumn(name="tlInv")
    private TLInventoryT tlInventoryT;
     @XmlTransient
    @OneToOne(cascade={CascadeType.ALL})
    ...
    @OneToOne(cascade={CascadeType.ALL})
    @JoinColumn(name="A_END_TP_FK")
    protected NamingAttributesT aEndTP;
    @OneToOne(cascade={CascadeType.ALL})
    @JoinColumn(name="Z_END_TP_FK")
    ...
    public void setTlInventoryT(TLInventoryT tlInventoryT) {
        this.tlInventoryT = tlInventoryT;
    }
